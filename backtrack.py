...
class GenerateurEDT13Semaines:
    ...
    def verifier_repartition(self, s, module, groupes_cible, max_par_semaine):
        count = 0
        for j in range(5):
            for c in range(4):
                for g in groupes_cible:
                    occ = self.planning[s][j][c][g]
                    if occ and occ[1] == module:
                        count += 1
    ...
    def resoudre(self, idx=0, reste=None):
        ...
        for s in range(13):
            for j in range(5):
                for c in range(4):
                    if self.verifier(s, j, c, curr['groupes'], curr['prof'], curr['dispos']) and self.verifier_repartition(s, curr['module'], curr['groupes'], max_par_semaine):
